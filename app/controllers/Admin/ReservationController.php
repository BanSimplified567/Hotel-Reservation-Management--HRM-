...
class ReservationController
{
    ...
    public function index()
    {
        ...
        require_once '../app/views/admin/reservations/index.php';
    }
    ...
    private function handleEditReservation($id)
    {
        ...
        if (empty($errors)) {
            try {
                ...
                header("Location: index.php?action=admin/reservations&action=view&id=$id");
                exit();
        ...
        if (!empty($errors)) {
            ...
            $_SESSION['old'] = $_POST;
            header("Location: index.php?action=admin/reservations&action=edit&id=$id");
            exit();
        }
    }
    ...
    private function showEditForm($id)
    {
        try {
            $stmt = $this->pdo->prepare("
                SELECT r.*,
                       u.username, u.email,
                       rm.room_number, rm.type as room_type, rm.capacity
                FROM reservations r
                JOIN users u ON r.user_id = u.id
                JOIN rooms rm ON r.room_id = rm.id
                WHERE r.id = ?
