<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Login - Hotel Bannie State Of Cebu System</title>
    <!-- Bootstrap 5 CSS -->
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.2/dist/css/bootstrap.min.css" rel="stylesheet">
    <!-- Bootstrap Icons -->
    <link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/bootstrap-icons@1.11.1/font/bootstrap-icons.css">
    <style>
        :root {
            --ph-blue: #003893;
            --ph-red: #ce1126;
            --ph-yellow: #fcd116;
            --ph-dark-blue: #002a6e;
            --ph-light-blue: #e8f1ff;
        }

        body {
            background: linear-gradient(135deg,
                    rgba(0, 56, 147, 0.05) 0%,
                    rgba(206, 17, 38, 0.05) 100%);
            font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif;
            min-height: 100vh;
            display: flex;
            align-items: center;
            justify-content: center;
            padding: 15px;
        }

        .login-container {
            max-width: 1000px;
            width: 100%;
        }

        .login-card {
            border: 1px solid rgba(0, 56, 147, 0.08);
            box-shadow: 0 10px 25px rgba(0, 56, 147, 0.12);
            overflow: hidden;
            animation: fadeInUp 0.6s ease-out;
        }

        @keyframes fadeInUp {
            from {
                opacity: 0;
                transform: translateY(20px);
            }
            to {
                opacity: 1;
                transform: translateY(0);
            }
        }

        .welcome-section {
            background: linear-gradient(135deg, var(--ph-blue) 0%, var(--ph-dark-blue) 100%);
            position: relative;
            overflow: hidden;
        }

        .welcome-section::before {
            content: '';
            position: absolute;
            top: -50%;
            left: -50%;
            width: 200%;
            height: 200%;
            background: radial-gradient(circle, rgba(255, 255, 255, 0.1) 1px, transparent 1px);
            background-size: 18px 18px;
            opacity: 0.08;
            pointer-events: none;
        }

        .logo-container {
            width: 120px;
            height: 120px;
            background: rgba(255, 255, 255, 0.95);
            border: 4px solid var(--ph-yellow);
        }

        .feature-card {
            background: rgba(255, 255, 255, 0.08);
            border: 1px solid rgba(255, 255, 255, 0.15);
            backdrop-filter: blur(4px);
            transition: transform 0.2s ease, background 0.2s ease;
        }

        .feature-card:hover {
            transform: translateX(5px);
            background: rgba(255, 255, 255, 0.12);
        }

        .feature-icon {
            animation: float 3s ease-in-out infinite;
            color: var(--ph-yellow);
        }

        .feature-card:nth-child(2) .feature-icon {
            animation-delay: 0.5s;
        }

        .feature-card:nth-child(3) .feature-icon {
            animation-delay: 1s;
        }

        @keyframes float {
            0%, 100% {
                transform: translateY(0);
            }
            50% {
                transform: translateY(-5px);
            }
        }

        .input-group-custom {
            border: 1.5px solid #e0e6f0;
            transition: all 0.25s ease;
        }

        .input-group-custom:focus-within {
            border-color: var(--ph-blue);
            box-shadow: 0 0 0 0.2rem rgba(0, 56, 147, 0.1);
        }

        .input-icon {
            background-color: #f8fafd;
            border-right: 1px solid #e0e6f0;
            color: var(--ph-blue);
        }

        .btn-login {
            background: linear-gradient(135deg, var(--ph-blue) 0%, var(--ph-dark-blue) 100%);
            border: none;
            transition: all 0.25s ease;
            box-shadow: 0 4px 10px rgba(0, 56, 147, 0.15);
        }

        .btn-login:hover {
            transform: translateY(-2px);
            box-shadow: 0 6px 15px rgba(0, 56, 147, 0.25);
        }

        .btn-login:active {
            transform: translateY(0);
        }

        .pulse {
            animation: pulse 2s infinite;
        }

        @keyframes pulse {
            0% {
                box-shadow: 0 4px 10px rgba(0, 56, 147, 0.15), 0 0 0 0 rgba(0, 56, 147, 0.15);
            }
            70% {
                box-shadow: 0 4px 10px rgba(0, 56, 147, 0.15), 0 0 0 8px rgba(0, 56, 147, 0);
            }
            100% {
                box-shadow: 0 4px 10px rgba(0, 56, 147, 0.15), 0 0 0 0 rgba(0, 56, 147, 0);
            }
        }

        .form-check-input:checked {
            background-color: var(--ph-blue);
            border-color: var(--ph-blue);
        }

        .text-ph-blue {
            color: var(--ph-blue) !important;
        }

        .text-ph-red {
            color: var(--ph-red) !important;
        }

        .border-ph-blue {
            border-color: var(--ph-blue) !important;
        }

        @media (max-width: 992px) {
            .login-card {
                max-width: 500px;
                margin: 0 auto;
            }

            .logo-container {
                width: 100px;
                height: 100px;
            }
        }

        @media (max-width: 576px) {
            body {
                padding: 10px;
            }

            .login-card {
                border-radius: 12px !important;
            }
        }
    </style>
</head>
<body>
    <div class="login-container">
        <div class="row g-0 login-card rounded-4">
            <!-- Left Column: Welcome/Info Section -->
            <div class="col-lg-6 welcome-section text-white p-4 p-lg-5 d-flex flex-column justify-content-center">
              <div class="position-relative z-1 text-center">
  <div
    class="logo-container rounded-circle d-flex align-items-center justify-content-center mx-auto mb-4 shadow bg-primary text-white">
   <i class="bi bi-building fs-1" style="color: var(--primary-blue);"></i>
  </div>

  <h1 class="h2 fw-bold mb-2">Hotel Bannie State Of Cebu System</h1>
  <p class="mb-5 opacity-90">Barangay Sibonga</p>
</div>

                <div class="position-relative z-1 mt-4">
                    <div class="row g-3">
                        <div class="col-12">
                            <div class="feature-card rounded-3 p-3">
                                <div class="d-flex align-items-center">
                                    <div class="feature-icon me-3">
                                        <i class="bi bi-shield-check fs-4"></i>
                                    </div>
                                    <div>
                                        <h6 class="fw-bold mb-1">Secure & Reliable</h6>
                                        <p class="small opacity-85 mb-0">Government-grade security for all your barangay data</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div class="col-12">
                            <div class="feature-card rounded-3 p-3">
                                <div class="d-flex align-items-center">
                                    <div class="feature-icon me-3">
                                        <i class="bi bi-speedometer2 fs-4"></i>
                                    </div>
                                    <div>
                                        <h6 class="fw-bold mb-1">Efficient Management</h6>
                                        <p class="small opacity-85 mb-0">Streamline barangay operations and citizen services</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div class="col-12">
                            <div class="feature-card rounded-3 p-3">
                                <div class="d-flex align-items-center">
                                    <div class="feature-icon me-3">
                                        <i class="bi bi-people-fill fs-4"></i>
                                    </div>
                                    <div>
                                        <h6 class="fw-bold mb-1">Community Focused</h6>
                                        <p class="small opacity-85 mb-0">Designed to serve the residents of Barangay Sibonga</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>

            <!-- Right Column: Login Form -->
            <div class="col-lg-6 bg-white p-4 p-lg-5 d-flex flex-column justify-content-center">
                <div class="text-center mb-4">
                    <h2 class="h3 fw-bold text-ph-blue mb-2">Welcome Back</h2>
                    <p class="text-muted">Sign in to access your dashboard</p>
                </div>

                <?php if (isset($_SESSION['error'])): ?>
                    <div class="alert alert-danger alert-dismissible fade show d-flex align-items-center mb-4" role="alert">
                        <i class="bi bi-exclamation-triangle-fill me-2"></i>
                        <div><?php echo htmlspecialchars($_SESSION['error']);
                            unset($_SESSION['error']); ?></div>
                        <button type="button" class="btn-close ms-auto" data-bs-dismiss="alert" aria-label="Close"></button>
                    </div>
                <?php endif; ?>

                <?php if (isset($_SESSION['success'])): ?>
                    <div class="alert alert-success alert-dismissible fade show d-flex align-items-center mb-4" role="alert">
                        <i class="bi bi-check-circle-fill me-2"></i>
                        <div><?php echo htmlspecialchars($_SESSION['success']);
                            unset($_SESSION['success']); ?></div>
                        <button type="button" class="btn-close ms-auto" data-bs-dismiss="alert" aria-label="Close"></button>
                    </div>
                <?php endif; ?>

                <form action="index.php?action=login" method="POST" id="loginForm" class="mb-4">
                    <div class="mb-4">
                        <label for="email" class="form-label fw-semibold text-ph-blue">Email Address</label>
                        <div class="input-group input-group-custom rounded-3">
                            <span class="input-icon input-group-text border-0 rounded-start-3">
                                <i class="bi bi-envelope-fill"></i>
                            </span>
                            <input type="email" class="form-control border-0 shadow-none" id="email" name="email"
                                autocomplete="email" required placeholder="barangay.staff@example.com">
                        </div>
                    </div>

                    <div class="mb-4">
                        <label for="password" class="form-label fw-semibold text-ph-blue">Password</label>
                        <div class="input-group input-group-custom rounded-3">
                            <span class="input-icon input-group-text border-0 rounded-start-3">
                                <i class="bi bi-lock-fill"></i>
                            </span>
                            <input type="password" class="form-control border-0 shadow-none" id="password" name="password"
                                autocomplete="current-password" required placeholder="Enter your password">
                            <button class="btn input-group-text border-0 rounded-end-3" type="button" id="togglePassword">
                                <i class="bi bi-eye"></i>
                            </button>
                        </div>
                    </div>

                    <div class="form-check mb-4">
                        <input class="form-check-input" type="checkbox" id="rememberMe" name="remember">
                        <label class="form-check-label text-muted" for="rememberMe">
                            Remember me on this device
                        </label>
                    </div>

                    <button type="submit" class="btn btn-login text-white w-100 py-2 rounded-3 fw-semibold pulse mb-4">
                        <i class="bi bi-box-arrow-in-right me-2"></i>Login to Dashboard
                    </button>
                </form>

                <div class="text-center pt-3 border-top">
                    <p class="text-muted mb-3">
                        New to the system?
                        <a href="index.php?action=register" class="text-ph-blue text-decoration-none fw-semibold">
                            <i class="bi bi-person-plus-fill me-1"></i>Create Account
                        </a>
                    </p>
                    <p class="mb-0">
                        <a href="index.php?action=forgot-password" class="text-ph-blue text-decoration-none fw-semibold">
                            <i class="bi bi-key-fill me-1"></i>Forgot Password?
                        </a>
                    </p>
                </div>
            </div>
        </div>
    </div>

    <!-- Bootstrap JS -->
    <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.2/dist/js/bootstrap.bundle.min.js"></script>
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            // Toggle password visibility
            const togglePassword = document.querySelector('#togglePassword');
            const password = document.querySelector('#password');

            if (togglePassword && password) {
                togglePassword.addEventListener('click', function() {
                    const type = password.getAttribute('type') === 'password' ? 'text' : 'password';
                    password.setAttribute('type', type);

                    const icon = this.querySelector('i');
                    if (icon) {
                        icon.classList.toggle('bi-eye');
                        icon.classList.toggle('bi-eye-slash');
                    }

                    // Focus back to password field
                    password.focus();
                });
            }

            // Auto-focus on email field
            const emailField = document.querySelector('#email');
            if (emailField) {
                emailField.focus();
            }

            // Form submission animation
            const loginForm = document.getElementById('loginForm');
            if (loginForm) {
                loginForm.addEventListener('submit', function(e) {
                    const submitBtn = this.querySelector('.btn-login');
                    if (submitBtn) {
                        submitBtn.innerHTML = '<span class="spinner-border spinner-border-sm me-2" role="status" aria-hidden="true"></span>Logging in...';
                        submitBtn.disabled = true;
                        submitBtn.classList.remove('pulse');
                    }
                });
            }

            // Feature cards hover effect
            const featureCards = document.querySelectorAll('.feature-card');
            featureCards.forEach(card => {
                card.addEventListener('mouseenter', function() {
                    const icon = this.querySelector('.feature-icon');
                    if (icon) icon.style.animationPlayState = 'paused';
                });

                card.addEventListener('mouseleave', function() {
                    const icon = this.querySelector('.feature-icon');
                    if (icon) icon.style.animationPlayState = 'running';
                });
            });
        });
    </script>
</body>
</html>
